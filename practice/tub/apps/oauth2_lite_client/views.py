...
def provider_cb(request, slug = None):
    ...
    if len(slug) > 0:
        ...
        credential, created = Credential.objects.get_or_create(user = request.user, provider = provider)
        code = request.GET.get('code', None)
        if code:
            print "code: %s" % code
            print "provider %s" % provider
            client = oauth2_client.Client(provider.client_id, provider.client_secret)
            access_token, refresh_token = client.redeem_code(refresh_uri = provider.token_url, redirect_uri = provider.redirect_url, code = code)
