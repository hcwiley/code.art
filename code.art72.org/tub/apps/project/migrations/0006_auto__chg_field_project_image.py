...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Renaming column for 'Project.image' to match new field type.
        db.rename_column('project_project', 'image', 'image_id')
        # Changing field 'Project.image'
        db.alter_column('project_project', 'image_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['project.ExtendedImage'], null=True))

        # Adding index on 'Project', fields ['image']
        db.create_index('project_project', ['image_id'])


    def backwards(self, orm):
        
        # Removing index on 'Project', fields ['image']
        db.delete_index('project_project', ['image_id'])

        # Renaming column for 'Project.image' to match new field type.
        db.rename_column('project_project', 'image_id', 'image')
        # Changing field 'Project.image'
        db.alter_column('project_project', 'image', self.gf('sorl.thumbnail.fields.ImageField')(max_length=100, null=True))
